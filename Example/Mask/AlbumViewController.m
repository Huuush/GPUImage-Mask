...
-(void)setupViewController {
    ...
    [PhotoManager standardPhotoManger].choiceCountChange = ^(NSInteger choiceCount) {
        ...
        if (choiceCount == 0) {
            ...
            [weakSelf.confirmButton setTitle:[NSString stringWithFormat:@"确定%ld/%ld", [PhotoManager standardPhotoManger].choiceCount, [PhotoManager standardPhotoManger].maxCount] forState:UIControlStateNormal];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.selectPhotoAction = ^(PHAsset *asset) {
        ...
        if ([weakSelf.albumModel.selectRows containsObject:@(indexPath.row)]) {
            ...
            [PhotoManager standardPhotoManger].choiceCount--;
            
            isReloadCollectionView = [PhotoManager standardPhotoManger].choiceCount == 9;
        } else {
            if ([PhotoManager standardPhotoManger].maxCount == [PhotoManager standardPhotoManger].choiceCount) {
                return;
            }
            
            [weakSelf.albumModel.selectRows addObject:@(indexPath.row)];
            [PhotoManager standardPhotoManger].choiceCount++;
            isReloadCollectionView = [PhotoManager standardPhotoManger].choiceCount == 10;
        }
...
-(void)confirmAction:(UIButton *)button {
    if ([PhotoManager standardPhotoManger].choiceCount > 0) {
        button.enabled = NO;
            ...
            for (NSNumber *row in albumModel.selectRows) {
                if (row.integerValue < albumModel.assets.count) {
                    ...
                    photoModel.getPictureAction = ^{
                        ...
                        if (photoList.count == [PhotoManager standardPhotoManger].choiceCount) {
                            button.enabled = YES;
                            
                            [PhotoManager standardPhotoManger].photoModelList = photoList;
