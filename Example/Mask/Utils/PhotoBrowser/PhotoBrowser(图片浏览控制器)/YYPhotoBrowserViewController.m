...
- (instancetype)initWithImageArray:(NSArray *)imageArray currentImageIndex:(int)currentImageIndex imageViewArray:(NSMutableArray *)imageViewArray imageViewFrameArray:(NSMutableArray *)imageViewFrameArray
{
    if (self = [self init])
    {
...
//        };
        ...
        _translation.imageArray = self.imageArray;
...
- (YYPhotoBrowserMainScrollView *)mainScrollView
{
    if (!_mainScrollView)
    {
        _mainScrollView = [[YYPhotoBrowserMainScrollView alloc] initWithFrame:CGRectMake(0, 0, self.view.yy_width, self.view.yy_height) imageArray:self.imageArray currentImageIndex:self.currentImageIndex];
