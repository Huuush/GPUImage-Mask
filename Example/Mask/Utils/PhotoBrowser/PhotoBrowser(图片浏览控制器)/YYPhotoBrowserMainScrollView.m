...
- (instancetype)initWithFrame:(CGRect)frame imageArray:(NSArray *)imageArray currentImageIndex:(int)index
{
...
- (void)createUI
{
    ...
    mainScrollView.contentSize = CGSizeMake(self.imageArray.count * (self.yy_width + MARGIN_BETWEEN_IMAGE), 0);// 设置UIScrollView的滚动范围（内容大小）
    ...
    {
        YYPhotoBrowserSubScrollView *subScrollView = [[YYPhotoBrowserSubScrollView alloc] initWithFrame:CGRectMake(i * (self.yy_width + MARGIN_BETWEEN_IMAGE), 0, self.yy_width, self.yy_height) image:self.imageArray[i]];
