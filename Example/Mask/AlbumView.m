...
typedef void(^AlbumViewSelectAction)(AlbumModel *albumModel);
...
/// 相册列表
@property (nonatomic, strong) AlbumTableView *tableView;
/// 列表的高度
...
-(void)setAssetCollectionList:(NSMutableArray<AlbumModel *> *)assetCollectionList {
    ...
    __weak typeof(self) weakSelf = self;
    self.tableView.selectAction = ^(AlbumModel *albumModel) {
...
#pragma mark - Get方法
-(AlbumTableView *)tableView {
    if (!_tableView) {
        _tableView = [[AlbumTableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) style:UITableViewStylePlain];
